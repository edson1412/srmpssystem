...
class CustomUserCreationForm(UserCreationForm):
    class Meta:
        model = CustomUser
        fields = ('username', 'email', 'first_name', 'last_name', 'role', 'rank', 'region', 'prison_station')
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        selected_role = self.data.get('role') if self.data else self.instance.role
        if selected_role in ['rco', 'rho', 'regional_data_control']:
            self.fields['region'].required = True
            self.fields['region'].help_text = 'Select the region this user manages.'
