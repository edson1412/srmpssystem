...
def dashboard(request):
    ...
    show_prisoner_stats = request.user.is_super_admin() or request.user.is_prison_admin() or \
                           request.user.is_reception() or request.user.is_warden() or request.user.is_visitor_attendant()
    ...
    if not request.user.is_super_admin() and hasattr(request.user, 'prison_station') and request.user.prison_station:
        prisoners = prisoners.filter(prison_station=request.user.prison_station)
    elif not request.user.is_super_admin() and (not hasattr(request.user, 'prison_station') or not request.user.prison_station):
    ...
    show_medical_stats = request.user.is_medical_officer() or request.user.is_super_admin() or request.user.is_prison_admin()
    ...
    show_ration_stats = request.user.is_super_admin() or request.user.is_prison_admin() or request.user.is_warden() or request.user.is_reception()
    ration_alerts = []
    ...
    if show_ration_stats and hasattr(request.user, 'prison_station') and request.user.prison_station:
        ...
        total_inmates_for_ration = active_prisoners_in_station.count()
        children_in_station = sum(
        ...
        total_people_requiring_ration = total_inmates_for_ration + children_in_station
...
def prisoner_list(request):
    ...
    if not (request.user.is_super_admin() or request.user.is_prison_admin() or
            request.user.is_reception() or request.user.is_visitor_attendant() or request.user.is_warden()):
...
def add_prisoner(request):
    ...
    if not (request.user.is_super_admin() or request.user.is_prison_admin() or request.user.is_reception()):
    ...
    if request.method == 'POST':
        ...
        if prisoner_form.is_valid():
            ...
            elif not prisoner.prison_station and request.user.is_superuser:
...
def add_convicted_details(request, prisoner_id):
    ...
    if not request.user.is_superuser and (not prisoner.prison_station or prisoner.prison_station != request.user.prison_station):
...
def prisoner_detail(request, prisoner_id):
    ...
    if not (request.user.is_super_admin() or request.user.is_prison_admin() or
            request.user.is_reception() or request.user.is_warden() or request.user.is_medical_officer()):
...
def transfer_prisoner(request, prisoner_id):
    ...
    if prisoner.prison_station == None and not request.user.is_super_admin():
...
def apply_sentence_reduction(request, prisoner_id):
    ...
    if not (request.user.is_super_admin() or request.user.is_warden() or request.user.is_prison_admin()):
...
def generate_prisoner_report(request, prisoner_id):
    ...
    if not (request.user.is_super_admin() or request.user.is_prison_admin() or
            request.user.is_reception() or request.user.is_warden()):
...
def upcoming_releases_report(request):
    ...
    if not (request.user.is_super_admin() or request.user.is_prison_admin() or
            request.user.is_warden() or request.user.is_reception()):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'prison_station') and request.user.prison_station:
    ...
    if report_format == 'pdf':
        ...
        context = {
            ...
            'user': request.user,
            'station_name': request.user.prison_station.name if hasattr(request.user, 'prison_station') and request.user.prison_station and not request.user.is_superuser else "All Stations"
        }
        html_string = render_to_string(template_path, context)
        response = HttpResponse(content_type='application/pdf')
...
def prison_statistics_api(request):
    ...
    if not request.user.is_superuser and not (hasattr(request.user, 'prison_station') and request.user.prison_station):
...
class VisitorUpdateView(RoleRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if 'approval_status' in form.changed_data or 'is_approved' in form.changed_data:
            if not (self.request.user.is_super_admin() or self.request.user.is_prison_admin() or self.request.user.is_warden() or self.request.user.has_perm('prison.can_approve_visit')):
...
class MedicalRecordDeleteView(RoleRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if not user.is_super_admin():
            if hasattr(user, 'prison_station') and user.prison_station and (user.is_medical_officer() or user.is_prison_admin()):
                return queryset.filter(prisoner__prison_station=user.prison_station)
            return queryset.none()
        return queryset
...
class ActivityLogListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def test_func(self):
        ...
        return self.request.user.is_superuser or self.request.user.is_prison_admin()
...
class PrisonerItemListView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        if not (user.is_super_admin() or user.is_prison_admin() or user.is_reception() or user.is_warden()):
...
class AddPrisonerItemView(RoleRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_superuser and (not self.prisoner.prison_station or self.prisoner.prison_station != request.user.prison_station):
...
class WithdrawPrisonerMoneyView(RoleRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Check permission for prisoner's station
        if not request.user.is_superuser and (not self.money_item.prisoner.prison_station or self.money_item.prisoner.prison_station != request.user.prison_station):
...
class CollectPrisonerItemView(RoleRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Permission check: Ensure user has rights for this prisoner's station or is superuser
        if not request.user.is_superuser and (not item.prisoner.prison_station or item.prisoner.prison_station != request.user.prison_station):
...
def extended_prisoner_search(request):
    ...
    if form.is_valid():
        ...
        if request.user.is_superuser and selected_prison_station:
            prisoners = prisoners.filter(prison_station=selected_prison_station)
        elif not request.user.is_superuser and hasattr(request.user, 'prison_station') and request.user.prison_station:
...
class RationItemListView(RoleRequiredMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not user.is_superuser and hasattr(user, 'prison_station') and user.prison_station:
        ...
        if form.is_valid():
            ...
            if user.is_superuser and not ration_item.prison_station:
...
class RationItemUpdateView(RoleRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if not self.request.user.is_superuser and hasattr(self.request.user, 'prison_station') and self.request.user.prison_station:
...
class RationConsumptionCreateView(RoleRequiredMixin, CreateView):
    """
    Allows recording daily consumption of ration items.
    Automatically calculates number of prisoners.
    """
    model = RationConsumption
    form_class = RationConsumptionForm
    template_name = 'prison/ration_consumption_form.html'
    success_url = reverse_lazy('ration_dashboard') # Redirect to dashboard after consumption
    roles_required = ['reception', 'warden', 'prison_admin', 'superuser']
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_people_requiring_ration'] = total_people_requiring_ration
        context['recommended_ration_per_person_kg'] = Decimal('0.680') # Display the constant in KG

        # Calculate recommended daily consumption for each item based on current population
        recommended_consumption_per_item = {}
        if user.prison_station:
            for item in RationItem.objects.filter(prison_station=user.prison_station, is_active=True):
                # Calculate the recommended quantity in kg for the entire population
                recommended_consumption_per_item[item.id] = Decimal(total_people_requiring_ration) * Decimal('0.680')
        context['recommended_consumption_per_item'] = recommended_consumption_per_item

        return context

    def form_valid(self, form):
        consumption = form.save(commit=False)
        consumption.consumed_by = self.request.user

        user = self.request.user
        # Calculate num_prisoners_fed based on the current station's active prisoners + children
        num_prisoners_fed = 0
        if hasattr(user, 'prison_station') and user.prison_station:
            active_prisoners_in_station = Prisoner.objects.filter(
                prison_station=user.prison_station,
            ...
            num_prisoners_fed = total_inmates_for_ration + children_in_station

        consumption.num_prisoners_fed = num_prisoners_fed # Set the number of prisoners fed

        # Auto-calculate quantity_used_kg based on the recommended amount
        # This will only work if an 'item' is selected.
        if consumption.item:
            # Assuming 0.680 kg per person per day
            consumption.quantity_used_kg = Decimal(consumption.num_prisoners_fed) * Decimal('0.680')
        else:
            # This case should ideally be caught by form validation if 'item' is required
            # but as a fallback, raise an error or set a default
            form.add_error('item', "Ration item must be selected for auto-calculation.")
            return self.form_invalid(form)


        try:
            consumption.save()
            messages.success(self.request, f"Daily consumption for {consumption.item.name} recorded successfully.")
            ActivityLog.objects.create(
                user=self.request.user, action='record_consumption', model='RationConsumption',
                object_id=consumption.id,
                details=f'Recorded {consumption.quantity_used_kg}kg of {consumption.item.name} for {consumption.num_prisoners_fed} people.'
            )
            return super().form_valid(form)
        except ValidationError as e:
            for field, errors in e.message_dict.items():
                for error in errors:
                    form.add_error(field if field != '__all__' else None, error)
            messages.error(self.request, "Failed to record consumption. Please correct the errors.")
            return self.form_invalid(form)
