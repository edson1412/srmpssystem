...
from .forms import (
    ...
    ReturnsFilterForm, PeriodSelectionForm, DatabaseReturnForm,
    MonthlyTrackingForm
)
from .services import (
    ReturnProcessingService, DefaultTemplateService,
    ReturnReportService, MonthlySubmissionService, DatabaseReturnGenerationService
)
from .pdf_service import ReturnPDFService
from prison.utils import create_return_submission_notification, create_return_approved_notification
...
def download_template(request, category):
    ...
    response.write(csv_content.encode('utf-8-sig'))  # Add BOM for Excel compatibility
...
def submission_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            create_return_submission_notification(submission)
...
@login_required
def database_submission_create(request):
    """Generate a return from prisoners registered at the user's station."""
    user = request.user
    station = _get_user_station(user)
    if not station and not user.is_superuser:
        messages.error(request, 'You must be assigned to a prison station to generate returns.')
        return redirect('returns:submission_list')

    categories = [
        (code, label) for code, label in ReturnTemplate.CATEGORY_CHOICES
        if code in DatabaseReturnGenerationService.SUPPORTED_CATEGORIES
    ]
    form = DatabaseReturnForm(request.POST or request.GET or None, categories=categories)
    category = request.POST.get('category') or request.GET.get('category', '')
    period = request.POST.get('period') or request.GET.get('period', timezone.now().strftime('%Y-%m'))
    candidates = []
    if category and station:
        candidates = DatabaseReturnGenerationService.get_prisoners(category, station)

    if request.method == 'POST' and form.is_valid() and 'preview' not in request.POST:
        if not station:
            messages.error(request, 'A prison station is required for generated returns.')
        elif not candidates:
            messages.error(request, 'No prisoners match this return type at your station.')
        else:
            child_details = {}
            if category in {'children_with_mothers', 'children_remand'}:
                for prisoner in candidates:
                    prefix = f'child_{prisoner.id}_'
                    name = request.POST.get(f'{prefix}name', '').strip()
                    age = request.POST.get(f'{prefix}age', '').strip()
                    sex = request.POST.get(f'{prefix}sex', '').strip()
                    if not name or not age or sex not in {'M', 'F'}:
                        messages.error(request, f'Enter complete child details for {prisoner.full_name}.')
                        break
                    child_details[str(prisoner.id)] = {'name': name, 'age': age, 'sex': sex}
                else:
                    submission = DatabaseReturnGenerationService.create_submission(
                        ReturnTemplate.objects.get(category=category, is_active=True),
                        station, user, period, child_details
                    )
                    MonthlySubmissionService.update_station_status(submission)
                    create_return_submission_notification(submission)
                    messages.success(request, f'Generated return with {submission.row_count} records.')
                    return redirect('returns:submission_detail', pk=submission.pk)
            else:
                submission = DatabaseReturnGenerationService.create_submission(
                    ReturnTemplate.objects.get(category=category, is_active=True),
    ...
    return render(request, 'returns/database_submission_form.html', {
        'form': form,
        'candidates': candidates,
        'selected_category': category,
        'selected_period': period,
        'requires_child_details': category in {'children_with_mothers', 'children_remand'},
    })
...
def submission_detail(request, pk):
    ...
    # Older database-generated submissions stored internal mapping aliases in
    # row_data. Rebuild their display data from the template headers.
    if submission.notes == 'Generated from the prisoner database.':
        headers = DatabaseReturnGenerationService._display_headers(
            submission.template.column_headers
        )
        for row in data_rows:
            raw_data = row.row_data
            row.row_data = {
                header: DatabaseReturnGenerationService._value_for_header(
                    header, raw_data, row.row_number, headers[:position]
                )
...
def submission_approve(request, pk):
    ...
    if request.method == 'POST':
        ...
        create_return_approved_notification(submission)
