...
class DatabaseReturnGenerationService:
    """Build return rows from the prison register using existing templates."""

    SUPPORTED_CATEGORIES = {
        'convicted_inmates', 'due_discharge', 'convicted_foreigners',
        'general_remand', 'remand_murder', 'elderly_inmates',
        'chronically_ill', 'discharged_reduction', 'children_with_mothers',
        'children_remand',
    }

    @classmethod
    def get_prisoners(cls, category, station):
        prisoners = Prisoner.objects.filter(
            prison_station=station,
            is_active=True,
        ).select_related('prison_station', 'particulars', 'physical', 'convicted_details', 'remand_details')

        if category in {'convicted_inmates', 'convicted_foreigners', 'elderly_inmates', 'chronically_ill', 'discharged_reduction', 'children_with_mothers'}:
            prisoners = prisoners.filter(prisoner_class='convicted')
        elif category in {'general_remand', 'remand_murder', 'children_remand'}:
            prisoners = prisoners.filter(prisoner_class='remand')

        if category == 'convicted_foreigners':
            prisoners = prisoners.exclude(particulars__nationality__in=['malawian', '', None])
        elif category == 'elderly_inmates':
            prisoners = prisoners.filter(age__gte=70)
        elif category in {'children_with_mothers', 'children_remand'}:
            prisoners = prisoners.filter(physical__has_child=True)
        elif category == 'remand_murder':
            prisoners = prisoners.filter(remand_details__offense__icontains='murder')
        elif category == 'chronically_ill':
            prisoners = prisoners.exclude(physical__health_status='none')
        elif category == 'discharged_reduction':
            prisoners = Prisoner.objects.filter(
                prison_station=station,
                prisoner_class='convicted',
                is_active=False,
                convicted_details__reduction_months__gt=0,
            ).select_related('prison_station', 'particulars', 'physical', 'convicted_details')

        return prisoners.order_by('prisoner_number')

    @classmethod
    def generate_rows(cls, category, station, child_details=None):
        if category not in cls.SUPPORTED_CATEGORIES:
            raise ValidationError('This return type cannot currently be generated from database records.')

        child_details = child_details or {}
        rows = []
        for index, prisoner in enumerate(cls.get_prisoners(category, station), start=1):
            convicted = getattr(prisoner, 'convicted_details', None)
            remand = getattr(prisoner, 'remand_details', None)
            particulars = getattr(prisoner, 'particulars', None)
            physical = getattr(prisoner, 'physical', None)
            offense = (convicted.offense if convicted else None) or (remand.offense if remand else '')
            court_case = ''
            sentence = ''
            date_of_committal = prisoner.date_admitted
            expiry_date = ''
            if convicted:
                court_case = f'{convicted.court} {convicted.case_number}'.strip()
                sentence = f'{convicted.sentence:g} months'
                date_of_committal = convicted.date_of_committal
                expiry_date = convicted.date_of_release_on_remission or convicted.date_of_release or ''
            elif remand:
                court_case = remand.court_case_number

            row = {
                's/n': index,
                'ser.no': index,
                'no': index,
                'pris.no': prisoner.prisoner_number,
                'pri.no': prisoner.prisoner_number,
                'name': prisoner.full_name,
                'particulars': cls._particulars(particulars),
                'sex': prisoner.get_sex_display().upper()[0],
                'age': prisoner.age,
                'offense': offense or '',
                'court_case_no.': court_case,
                'court/case.no': court_case,
                'sentence': sentence,
                'date_of_committal': cls._date(date_of_committal),
                'date_of_admission': cls._date(prisoner.date_admitted),
                'expiry_date_release_with_remission': cls._date(expiry_date),
                'expected_date_of_release': cls._date(expiry_date),
            }

            if category in {'children_with_mothers', 'children_remand'}:
                child = child_details.get(str(prisoner.id), {})
                row.update({
                    'name_of_a_child': child.get('name', ''),
    ...
    @classmethod
    def create_submission(cls, template, station, user, period, child_details=None):
        """Create a normal return submission from current prisoner records."""
        year, month = [int(value) for value in period.split('-', 1)]
        rows = cls.generate_rows(template.category, station, child_details)
        submission = ReturnSubmission.objects.create(
            template=template,
            prison_station=station,
            submitted_by=user,
            year=year,
            ...
            processed_by=user,
            notes='Generated from the prisoner database.',
        )
        ...
        for index, row in enumerate(rows, start=1):
            writer.writerow([cls._value_for_header(header, row, index, headers[:position]) for position, header in enumerate(headers)])
            sex = str(row.get('sex', '')).upper()
            display_row = {
                header: cls._value_for_header(header, row, index, headers[:position])
                for position, header in enumerate(headers)
            }
            ReturnData.objects.create(
                submission=submission,
                row_data=display_row,
                row_number=index,
                prisoner_number=str(row.get('pris.no', '')),
                name=str(row.get('name', '')),
                sex=sex,
                age=ReturnProcessingService._parse_int(str(row.get('age', ''))),
                offense=str(row.get('offense', '')),
                court_case_number=str(row.get('court_case_no.', '')),
                sentence=str(row.get('sentence', '')),
                date_of_committal=ReturnProcessingService._parse_date(str(row.get('date_of_committal', ''))),
                expiry_date=ReturnProcessingService._parse_date(str(row.get('expiry_date_release_with_remission', ''))),
            )

        submission.file.save(
            f'generated_return_{template.category}_{period}.csv',
            ContentFile(output.getvalue().encode('utf-8-sig')),
            save=False,
        )
        submission.row_count = len(rows)
        submission.total_male = sum(1 for row in rows if str(row.get('sex', '')).upper() == 'M')
        submission.total_female = sum(1 for row in rows if str(row.get('sex', '')).upper() == 'F')
        submission.save()
        return submission

    @staticmethod
    def _value_for_header(header, row, index, previous_headers=None):
        normalized = re.sub(r'[^a-z0-9]+', '_', header.lower()).strip('_')
        previous_headers = previous_headers or []
        aliases = {
            's_n': 's/n', 'ser_no': 'ser.no', 'no': 'no',
            'pris_no': 'pris.no', 'pri_no': 'pri.no',
            'court_case_no': 'court_case_no.', 'court_case_no_': 'court_case_no.',
            'court_case_no': 'court_case_no.', 'court': 'court_case_no.',
            'date_of_conv': 'date_of_committal', 'doc': 'date_of_committal',
            'd_o_c': 'date_of_admission', 'date_of_admission': 'date_of_admission',
            'e_d_r': 'expiry_date_release_with_remission',
            'expected_date_of_release': 'expected_date_of_release',
            'name_of_a_child': 'name_of_a_child',
        }
        key = aliases.get(normalized, normalized)
        if normalized == 'age' and any(re.sub(r'[^a-z0-9]+', '_', value.lower()).strip('_') == 'age' for value in previous_headers):
            return row.get('child_age', '')
        if normalized == 'sex' and any(re.sub(r'[^a-z0-9]+', '_', value.lower()).strip('_') == 'sex' for value in previous_headers):
            return row.get('child_sex', '')
        return row.get(key, row.get(normalized, ''))
    ...
    @staticmethod
    def _particulars(particulars):
        if not particulars:
            return ''
        return f'V: {particulars.village} D: {particulars.district}'
